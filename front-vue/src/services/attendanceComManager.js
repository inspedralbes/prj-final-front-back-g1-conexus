const BACK_URL=import.meta.env.VITE_ASSISTENCE_URL;

export const updateAttendance = async (courseId, userId, hour, assisted) => {
    try {
        const response = await fetch(`${BACK_URL}api/assistence`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ courseId, userId, hour, assisted }),
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
        return data;
    } catch (error) {
        console.error('Error fetching courses:', error);
    }
}
export const getAttendanceFromCourse = async (courseId) => {
    try {
        const response = await fetch(`${BACK_URL}api/assistences/course/${courseId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
        return data;
    } catch (error) {
        console.error('Error fetching courses:', error);
    }
}

export const getAlumns=async (courseId) => {
    try {
        const response = await fetch(`${BACK_URL}api/user-courses/${courseId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
        return data;
    } catch (error) {
        console.error('Error fetching courses:', error);
    }
}