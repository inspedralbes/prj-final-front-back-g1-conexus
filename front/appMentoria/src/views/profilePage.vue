  ...
  <div v-if="user.value" class="bg-gray-100 dark:bg-gray-900 min-h-screen">
    <section class="relative pt-40 pb-24 flex flex-col justify-between">
      ...
      <div class="w-full max-w-7xl mx-auto px-6 md:px-8 flex-grow relative">
        <!-- Profile image -->
        <div
          class="flex items-center justify-center sm:justify-start relative mb-5"
        >
          <img
            :src="`${profile}`"
        ...
        <!-- User Info -->
        <div
          class="flex flex-col sm:flex-row items-center justify-between mb-5"
        >
          <div class="block text-center sm:text-left">
            <h3
              class="font-manrope font-bold text-2xl text-gray-900 mb-1 dark:text-white"
            >
              {{ user.value.name }}
            </h3>
            <p
        ...
        <!-- Tags -->
        <div
          class="flex flex-col lg:flex-row items-center justify-between py-0.5"
        >
          <div
            class="flex flex-col md:flex-row items-center sm:justify-end gap-6"
          >
            <ul class="flex items-center flex-wrap justify-center gap-2.5">
              <li
                v-for="tag in user.value.tags"
                :key="tag"
      ...
      <div
        class="mt-5 w-full max-w-7xl mx-auto px-6 md:px-8 flex justify-center sm:justify-end gap-4"
      >
        <a
          v-if="user.value.discord_link"
          :href="`${user.value.discord_link}`"
          target="_blank"
