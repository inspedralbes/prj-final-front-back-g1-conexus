  ...
  <div class="flex flex-col min-h-screen">
    ...
    <main class="flex-grow p-4">
      <h1 class="py-5 text-4xl font-bold text-textThemeColor">
        Crear Nueva Coversacion
      </h1>
      <div class="border-buttonColorPrimary border-2 p-4 rounded-lg">
        <h2 class="text-lg font-semibold mb-2 text-textThemeColor">
          Llistat d'usuaris
        </h2>
        <input
          type="text"
        ...
        <ul>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <li
              ...
              :key="user.id"
              class="mb-1 flex items-center border-1 border-buttonColorPrimary"
...
const createChat = (userId) => {
  ...
  const confirmation = confirm(
    `Vols iniciar una nova conversa amb: "${user.name}"?`
  );
  const name = "";
  if (confirmation) {
